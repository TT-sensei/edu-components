...
export class TimeAttack {
  ...
  pause() { this.timer.pause(); return this; }
  resume() { this.timer.resume(); return this; }
  reset() { this.active = false; this.questionsAnswered = 0; this.correctCount = 0; this.currentQuestion = null; this.scoreManager?.reset?.(); this.comboManager?.reset?.(); this.timer.reset(this.duration); return this; }
  nextQuestion(criteria = {}) { this.currentQuestion = this.questionPool?.next(criteria) || null; return this.currentQuestion; }
  submit(result) { if (!this.active || result?.ignored) return false; if (typeof result === 'boolean') emit(this.eventTarget, result ? EDU_EVENTS.CORRECT : EDU_EVENTS.WRONG, { source: 'TimeAttack' }); return true; }
  ...
  finish() { if (!this.active) return this.getResult(); this.active = false; const result = this.getResult(); emit(this.eventTarget, EDU_EVENTS.COMPLETE, result); return result; }
